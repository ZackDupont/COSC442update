...
local actualCW = display.actualContentWidth
local actualCH = display.actualContentHeight
...
local dropCoins, dropDebt, onMenuTouch, onGlobalCollision -- forward declares for functions
local tmr_startGame, tmr_dropCoins, tmr_dropDebt -- forward declares for our timers
...
local debt = {} -- a table to store the debt objects (wrong coin)
local debtCounter = 1
...
dropCoins = function()
  physics.start()
  coins[coinCounter] = display.newImageRect("images/coin(rightOne).png", 110, 110)
    coins[coinCounter].x = math.random(left, right)
    coins[coinCounter].y = top - 60
    physics.addBody(coins[coinCounter], {radius=35})
...
dropDebt = function()
physics.start()
  debt[debtCounter] = display.newImageRect("images/coin(wrongOne).png", 110, 110)
  debt[debtCounter].x = math.random(left, right)
  debt[debtCounter].y = top - 60
  physics.addBody(debt[debtCounter], {radius = 35})
  debt[debtCounter].myName = "debt"
...
onGlobalCollision = function(event)
    ...
    local function fadeObject(obj) -- fade the coins out when they hit the floor
      obj.myName = "lostCoin"
      decrementScore()
      transition.to(obj, {alpha = 0, time = 50, onComplete=function(self) display.remove(self); end})
    end

    local function coinCatcher(obj)
      obj.myName = "caughtCoin"
      transition.to(obj, {alpha = 0, time = 50, onComplete=function(self) display.remove(self); end})
    end
    ...
    if ( event.phase == "began" ) then
      if(obj1.myName == "debt" and obj2.myName == "player") then -- listen for coins/floor collision   
        timer.performWithDelay(1, function() fadeObject(obj1); end, 0) -- don't ever remove an object in the middle of collision detection. Wait until the collision is over
      end
      if(obj1.myName == "player" and obj2.myName == "debt") then -- listen for floor/coins collision
      ...
      if(obj1.myName == "floor" and obj2.myName == "coins") then
        timer.performWithDelay(1, function() coinCatcher(obj2); end, 1)
      end

      if(obj1.myName == "coins" and obj2.myName == "floor") then
       timer.performWithDelay(1, function() coinCatcher(obj1); end, 1)
      end

      if(obj1.myName == "debt" and obj2.myName == "floor") then
       timer.performWithDelay(1, function() coinCatcher(obj1); end, 1)
      end

      if(obj1.myName == "floor" and obj2.myName == "debt") then
       timer.performWithDelay(1, function() coinCatcher(obj2); end, 1)
      end
...
function scene:create( event )
  ...
  local background = display.newImageRect(sceneGroup, "images/cbackground.jpg", actualCW, actualCH)
  	background.x = contentX
  ...
  local floorPhysics = display.newRect(sceneGroup, 0, 0, actualCW, 50)
    floorPhysics.x = contentX
  ...
  local leftWall = display.newRect(sceneGroup, 0, 0, 40, actualCH)
    leftWall.x = left - 25
    leftWall.y = contentY
    leftWall.myName = "wall"
    physics.addBody(leftWall, "static")

  local rightWall = display.newRect(sceneGroup, 0, 0, 40, actualCH)
    rightWall.x = right + 25
    rightWall.y = contentY
...
local word = display.newText(sceneGroup, "Animal: L I _ N", 160, 20, native.systemFontBold, 100)
  word.x = display.contentCenterX
  word.y = 100
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
      ...
      tmr_dropCoins = timer.performWithDelay(1200, dropCoins, 0)
      tmr_dropDebt = timer.performWithDelay(1200, dropDebt, 0) -- start dropping coins!

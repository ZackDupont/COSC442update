...
function scene:create( event )
  ...
  balance.rotation = 90
	...
	local word = display.newText(uiGroup, "K I T _", 1300, 2000, native.systemFont, 240)
	word.rotation = 90
	...
	local function endGame( event )
		...
		if not file then
		    ...
		    file:write("Level 4 completed in "..timeTaken.." seconds\n")
		end
		transition.cancel()
		composer.removeScene("level4")
	...
	local function listener( event )
		...
		elseif distance(fishI) < 60 then
			...
			if not file then
		    	...
		    	file:write("Wrong fish chosen: I\n")
			end
		...
		elseif distance(fishE) < 60 then
			...
			sceneGroup:insert(balance)
			display.remove(word)
			word = display.newText(uiGroup, " K I T E ", 1300, 2000, native.systemFont, 240)
			word.rotation = 90
			word:setFillColor(0,0,0)
			sceneGroup:insert(word)
